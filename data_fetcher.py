...
class DataFetcher():
    ...
    def poll_aws_data(self, symbol, start_time=9.5, end_time=16, days_to_poll=1, skip_today=False):
        print("Fetching aws")
        days = self.get_days_to_compute(days_to_poll, skip_today)
    ...
    def get_days_to_compute(self, days_to_poll, skip_today):
        ...
        return [day for day in month if not skip_today or day.strftime("%Y-%m-%d") != datetime.now().strftime("%Y-%m-%d")]