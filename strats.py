...
def main():
    ...
    parser.add_argument("--src", default="aws", help="Data source", choices={"yfinance", "aws"})
    parser.add_argument("--days", default=20, type=int, help="Number of days to compute")
    ...
    parser.add_argument("--brange", default=0, type=int, help="Buy max percent range to test")
    parser.add_argument("--srange", default=0, type=int, help="Sell max percent range to test")
    parser.add_argument("--include-partial-today", action='store_true', help="Skip todays info")
    parser.add_argument("--cpu", action='store_true', help="Use CPU instead of gpu")
    args = parser.parse_args()

    data_cache = DataFetcher()

    days = data_cache.poll_data(args.src, args.symbol, args.start, args.end, args.days, args.include_partial_today)
    if args.cpu:
...
def persist_results(symbol, highest_profit, highest_profit_b, highest_profit_s, transactions, invested_time, start_time, rbm, rsm, memory, max_val, key_val, computed_days, data_source):
    ...
    print(info_str)
    info = {"info": info_str, "symbol":symbol, "profit":highest_profit, "buy":highest_profit_b, "sell":highest_profit_s, "transactions":transactions, "invsted_time":invested_time, "elapsed_time": time.time() - start_time, "range_buy": rbm, "range_sell": rbm, "days_nb": len(computed_days), "days": [d.strftime("%Y-%m-%d") for d in computed_days], "data_source":data_source, "memory":memory, "multiday_profit": max_val, "multiday_buy": key_val.split('_')[0], "multiday_sell": key_val.split('_')[1], "multiday_rangeB": key_val.split('_')[2], "multiday_rangeS": key_val.split('_')[3]}
...
def analyse_results(results, symbol, start_time, computed_days, data_source, memory):
    ...
    for k in compiled.keys():
        if compiled[k] > max_val:
            max_val = compiled[k]
            key_val = k
    persist_results(symbol, highest_profit, highest_profit_b, highest_profit_s, transactions, invested_time, start_time, rbm, rsm, memory, max_val, key_val, computed_days, data_source)
    #gen_csv(compiled)
    #show_graph(compiled)
